...
psych_summary_FYCs <- left_join(FYC_dfs, summarized_psych_visits, by = c('DUPERSID', 'year')) %>% 
 left_join(charlson_score, by = c('DUPERSID', 'year'))

write_dta(psych_summary_FYCs, 'F:/projects/telepsych/data/ml_ready/psych_summary_FYC.dta')
