log using "F:/projects/telepsych/output/summary_fyc_mental_health_analysis.log", replace 

use "F:/projects/telepsych/data/ml_ready/mental_health_summary_FYC.dta", clear 

*drop unknown age 7023 --> 7000 obs 
drop if age < 0 
...
gen telemed = 0
replace telemed = 1 if total_mental_telehealth_visits > 0
...
global y_vars total_mental_health_visits total_mental_telehealth_visits oop_mental_health_visits total_spend_mental_health_visits tele_oop_mental_health_visits tele_total_spend_mh_visits

global spend_vars tele_oop_mental_health_visits tele_total_spend_mh_visits

global x_vars i.age_cat i.year i.SEX i.race i.edu i.region i.poverty i.insurance charlson_score autism_dx intellectual_dx personality_dx manic_episode_dx bipolar_disorder_dx mdd_dx persistent_mood_disorder_dx adhd_dx
...
svy: mean  $y_vars i.age_cat $id_vars 
...
foreach y in $spend_vars{
	display "Dependent Var `y'"
	svy: twopm `y' $x_vars ,  f(probit) s(glm, link(log) family(gamma))
	margins, dydx(insurance edu poverty SEX race)
	margins insurance edu poverty SEX race
}
...
esttab using "F:\projects\telepsych\output\table1_mental_health.rtf" , b(2) ci(2) label replace nostar 
